...
public class FileContextProviderTests
{
    ...
    public async Task GetContextsForFileTestsAsync(string workspaceRootRel, string filePathRel)
    {
        ...
        var fcp = new FileContextProvider(TestHelpers.MS(workspaceRoot), Mock.Of<IToolchainService>(), Mock.Of<IBuildOutputSink>(), GetSettingsService(), Mock.Of<IWorkspaceContextAccessor>());
